...
import {DragEndEvent ,useDroppable, useDraggable, useDndMonitor} from '@dnd-kit/core'
  ...
  const {elements, addElement, selectedElement, setSelectedElement, removeElement} = useDesigner();
    ...
    useDndMonitor({
      onDragEnd : (event: DragEndEvent) => {
        ...
        const draggingDesignerElelementOverAnotherDesignerElement = isDroppingOverDesignerElement && isDraggingDesignerElement;
        if(draggingDesignerElelementOverAnotherDesignerElement){
  ...
  return (
    <div className="flex w-full h-full">
      <div className="p-4 w-full" onClick={()=>{
        ...
        <div ref={droppable.setNodeRef} className={cn("bg-background max-w-[920px] h-full m-auto rounded-xl flex flex-col flex-grow items-center justify-start flex-1 overflow-y-auto", 
...
function DesignerElementWrapper({element}: {element: FormElementInstance}){
  ...
  >
        ...
        {
          topHalf.isOver && (
            <div className="absolute top-0 w-full bg-primary rounded-md h-[7px] rounded-b-none"/>
          )
        }
        <div className={cn("flex w-full h-[120px] items-center rounded-md bg-accent/40 px-4 py-2 pointer-events-none opacity-100",
          mouseIsOver && "opacity-30", mouseIsOver && "opacity-30"
          )}>
      <DesignerElement elementInstance={element}/>
      {
          bottomHalf.isOver && (
            <div className="absolute bottom-0 w-full bg-primary rounded-md h-[7px] rounded-t-none"/>
          )
        }
    </div>
  </div>
